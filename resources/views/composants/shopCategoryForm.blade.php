<form action="{{ route('shopCategoryFilter')}}" class="flex flex-col gap-2 py-4 border-b border-amber-700 border-opacity-20">
  ...
  @csrf
  <label for="typeBijou"><input type="radio" name="typeBijou" value="Anneau" onchange="this.form.submit()" class="focus:ring-second text-second"
    @if(isset($typeBijou)) {{ ($typeBijou == 'Anneau')?'checked' : ''}} @endif >&nbsp; Anneau</label>
  <label for="typeBijou"><input type="radio" name="typeBijou" value="Collier" onchange="this.form.submit()" class="focus:ring-second text-second"
    @if(isset($typeBijou)) {{ ($typeBijou == 'Collier')?'checked' : ''}} @endif >&nbsp; Collier</label>
  <label for="typeBijou"><input type="radio" name="typeBijou" value="Bracelet" onchange="this.form.submit()" class="focus:ring-second text-second"
