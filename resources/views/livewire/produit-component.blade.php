<div>
    ...
    <div class="w-full bg-third pt-3">
        ...
        @if(session('success'))
          <div class="alert alert-success max-sm:fixed inset-0 max-sm:mt-24 sm:max-w-xl max-sm:w-fit h-fit z-30 mx-auto max-sm:px-3 flex">
              <svg xmlns="http://www.w3.org/2000/svg" class="stroke-current shrink-0 h-6 w-6" fill="none" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
              <span>{{ session('success') }}</span>
          </div>
        @endif
        @if(session('error'))
          <div class="alert alert-error max-sm:fixed inset-0 max-sm:mt-24 sm:max-w-xl max-sm:w-fit h-fit z-30 mx-auto max-sm:px-3 flex">
