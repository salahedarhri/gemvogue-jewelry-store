<!DOCTYPE html>
<html lang="en" data-theme="autumn">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <meta http-equiv="X-UA-Compatible" content="ie=edge">

  <title>gemVogue</title>

  <!-- Tailwind & Fonts -->
  @vite('resources/css/app.css')
  <link href="https://fonts.googleapis.com/css2?family=Lato:ital,wght@0,300;0,400;0,600;0,700;1,300;1,400;1,700&display=swap" rel="stylesheet">
  <link href="https://fonts.googleapis.com/css2?family=Tajawal:wght@200;300;400;500&display=swap" rel="stylesheet">

</head>
<body>

  <!-- Barre Navigation -->
  <div class="navbar bg-base-100">
    <div class="flex-1 ">
        <a href="/"><img src="images/ring-logo (2).png" class="h-9 w-auto ml-2" alt="logo"></a>
        <a href="/" class="font-semibold normal-case text-xl max-sm:text-lg p-2">GemVogue</a>
    </div>

    <div class="flex-none">

      <!-- for medium and big screen -->
      <ul class="menu menu-horizontal px-1 max-md:hidden">
        <li><a href="/" class="text-base">Accueil</a></li>
        <li><a href="/boutique" class="text-base">Boutique</a></li>
        <li><a href="/apropos" class="text-base">à propos</a></li>
        @auth
        <li><a href="/dashboard" class="text-base">Espace Client</a></li>
        @else
        <li><a href="{{ route('login') }}" class="text-base">Connexion</a></li>
        <li><a href="{{ route('register') }}" class="text-base">Inscription</a></li>
        @endauth

      </ul>

      <!-- medium to small screen -->
      <div class="dropdown dropdown-bottom dropdown-end md:hidden">
        <label tabindex="0"><svg xmlns="http://www.w3.org/2000/svg"  viewBox="0 0 50 50" width="150px" height="150px" class="w-8 h-8 m-2 fill-neutral"><path d="M 5 8 A 2.0002 2.0002 0 1 0 5 12 L 45 12 A 2.0002 2.0002 0 1 0 45 8 L 5 8 z M 5 23 A 2.0002 2.0002 0 1 0 5 27 L 45 27 A 2.0002 2.0002 0 1 0 45 23 L 5 23 z M 5 38 A 2.0002 2.0002 0 1 0 5 42 L 45 42 A 2.0002 2.0002 0 1 0 45 38 L 5 38 z"/></svg></label>
        <ul tabindex="0" class="dropdown-content z-[1] menu p-2 shadow bg-base-100 rounded-box w-52 max-sm:w-52 max:sm">
          <li><a href="/" class="text-base" >Accueil</a></li>
          <li><a href="/boutique" class="text-base" >Boutique</a></li>
          <li><a href="/apropos" class="text-base">à propos</a></li>
          @auth
          <li><a href="/dashboard" class="text-base">Espace Client</a></li>
          @else
          <li><a href="{{ route('login') }}" class="text-base" >Connexion</a></li>
          <li><a href="{{ route('register') }}" class="text-base" >Inscription</a></li>
          @endauth

        </ul>
      </div>

    </div>
  </div>

  @yield('content')
  
  <footer class="footer max-md:footer-center items-center p-4 bg-neutral text-neutral-content">
    <aside class="items-center grid-flow-col">
      <img src="images/ring-logo (2).png" alt="logo" class="w-auto h-9 mx-1 invert">
      <p>gemVogue © 2023 - All right reserved</p>
    </aside> 
  </footer>
</body>
</html>