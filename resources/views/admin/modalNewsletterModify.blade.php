<div class="fixed inset-0 bg-slate-700 bg-opacity-50 flex items-center justify-center max-md:px-4 z-10">
    ...
    <!-- Modal -->
    <div  @click.away="openModalModify=false" class="lg:w-2/5 md:w-3/5 max-md:w-full bg-white shadow-lg rounded border-2 border-second border-opacity-50 p-3 font-dmsans">
        ...
        <form wire:submit.prevent="ModifierEmail" wire:key="email-modify-{{ $email }}" class="max-w-xl mx-auto flex flex-col gap-6 max-sm:gap-4 py-3 font-dmsans">
            ...
            <label for="email" class="w-full flex flex-col">
                ...
                <input type="text" name="email" wire:model.live="email" value="{{ $email }}"
            ...
            <div class="flex flex-row justify-between p-2">
                <button @click="openModalModify=false" class="w-fit text-white font-dmsans uppercase font-semibold px-4 py-2 bg-slate-500 rounded shadow hover:bg-slate-800 transition">Annuler</button>
